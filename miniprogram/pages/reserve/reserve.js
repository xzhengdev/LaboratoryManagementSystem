...
Page({
  data: {
    lab: {},
    schedule: [],
    form: { date: today(), start_time: '09:00', end_time: '11:00', participant_count: 1, purpose: '' },
    loading: false,
  ...
  onStartTime(e) { this.setData({ 'form.start_time': e.detail.value }) },
  onEndTime(e)   { this.setData({ 'form.end_time': e.detail.value }) },
  onPurpose(e)   { this.setData({ 'form.purpose': e.detail.value }) },
  incCount() {
  ...
  async doSubmit() {
    ...
    if (!form.purpose.trim()) {
      wx.showToast({ title: '请填写使用目的', icon: 'none' }); return
    }
